...
const initialTeams = [
  { initials: 'GA', name: 'Garuda Offroad', members: 5, motto: 'Jelajah tanpa batas!', color: '#2e9d63', total: 0 },
  { initials: 'NA', name: 'Naga Liar', members: 4, motto: 'Bakar semangat, taklukkan medan.', color: '#e8833a', total: 0 },
  { initials: 'EL', name: 'Elang Penjelajah', members: 5, motto: 'Tinggi terbang, dalam mengarungi.', color: '#1d7ce2', total: 0 },
];
...
type TeamIconType = 'users' | 'plus' | 'trash' | 'flag' | 'check';
...
function TeamIcon({ type }: { type: TeamIconType }) {
  ...
  if (type === 'trash') return <svg viewBox="0 0 24 24" width="17" height="17" {...common}><path d="M5 7h14M10 11v6M14 11v6M9 7V4h6v3M7 7l1 13h8l1-13" /></svg>;
  if (type === 'flag') return <svg viewBox="0 0 24 24" width="15" height="15" {...common}><path d="M5 21V4M5 5c4-3 7 3 14 0v9c-7 3-10-3-14 0" /></svg>;
  if (type === 'check') return <svg viewBox="0 0 24 24" width="15" height="15" {...common}><circle cx="12" cy="12" r="8.5" /><path d="m8.5 12 2.2 2.2 4.8-5" /></svg>;
  ...
  useEffect(() => {
    ...
    const loadTeams = async () => {
      try {
        ...
        const payload: { data?: Array<{ id: string; name: string; initials: string; motto: string }> } = await response.json();
        if (payload.data) {
        ...
        // Keep the local fallback list when the team request fails.
      }
    };
  ...
  const handleAddTeam = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/teams`, {
        ...
        credentials: 'include',
        body: JSON.stringify({ name: name.trim(), initials, motto: motto.trim() }),
      });
      ...
      setTeamList((current) => [...current, { id: payload.data.id, initials, name: name.trim(), members: Number(members), motto: motto.trim(), color, total: 0 }]);
      setName('');
      setMembers('');
      setMotto('');
      setShowForm(false);
    ...
    <div className="page-shell teams-shell" style={styles.appShell}>
      ...
      <main className="page-main teams-main" style={styles.mainContent}>
        ...
        <div className="pageHeader" style={styles.pageHeader}>
          ...
          <p className="subtitle" style={styles.subtitle}>Kelola tim yang bertualang di rute jeep.</p>
        </div>
        ...
        {showForm ? (
          <div style={styles.formCard}>
            ...
            {error ? <div style={styles.formError}>{error}</div> : null}
          ...
          {teamList.map((team) => (
            <div className="card" key={team.id || team.name} style={styles.card}>
              <div style={{ ...styles.cardTop, background: team.color }}>
                ...
                <div style={styles.cardTitleWrap}><div style={styles.cardTitle}>{team.name}</div><div style={styles.cardMeta}>{team.members} anggota</div></div>
                <button type="button" aria-label={`Hapus tim ${team.name}`} title={`Hapus tim ${team.name}`} onClick={() => handleDeleteTeam(team)} disabled={!team.id || deleting === team.id} style={styles.deleteButton}><TeamIcon type="trash" /></button>
              </div>
              <div style={styles.motto}>“{team.motto}”</div>
              <div style={styles.footerRow}><div style={styles.inlineMeta}><TeamIcon type="flag" /> 0/6 pos</div><div style={styles.inlineMeta}><TeamIcon type="check" /> game selesai</div><div style={styles.totalValue}>{team.total}</div></div>
            </div>
          ))}
        </div>
      </main>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  formError: { color: '#9c2d22', background: '#fff0ed', border: '1px solid #efc4bd', borderRadius: 8, padding: '8px 10px', fontSize: 12, marginBottom: 12 },
  ...
  deleteButton: { border: 'none', background: 'transparent', color: 'inherit', padding: 4, fontSize: 20, opacity: 0.85, cursor: 'pointer', display: 'inline-flex' },
  motto: { padding: '18px 14px 10px', fontSize: 16, color: '#3c4b4a', minHeight: 56 },
